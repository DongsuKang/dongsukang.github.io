...
## 대각화란?
이렇게 나에게 있어 사연 많은 대각화. 그래서 대각화가 도대체 무엇이고 부터 한 번 보자.  
...
수식을 통해 보면 알겠지만, 여기서 $\lambda$는 고윳값(eigenvalue), $\mathbf{v}$는 고유벡터(eigenvecotr)를 의미한다.
...
A를 대각화 해서 얻은 행렬을 D라고 하면, A와 D는 서로 닮음(similar) 관계이다. n-dimension 실수를 구성하는 서로 다른 기저이기 때문이다. D는 고유벡터로 이루어진 기저이고, A는 그 외의 임의의 기저이다. 두 기저는 같은 벡터 공간을 spanning 한다.
...
여기까지 읽고나면, 이제 행렬의 대각화는 고윳값과 고유벡터와 밀접한 관련이 있음을 알 수 있다. 그렇다면 행렬의 고윳값은 어떻게 구할까?
...
따라서,고유벡터가  $A-\lambda_{-2}I$과 곱해졌을 때 0이 되야하는 이유는 그냥 고유벡터의 정의 자체가 그렇기 때문이다.
